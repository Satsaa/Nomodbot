...
// This tmi.js client file version is from user dbkynd
...
client.prototype.handleMessage = function handleMessage(message) {
    if (!_.isNull(message)) {
        ...
        // Emit raw message to help facilitate client logging
        this.emit('raw', channel, message);
        ...
        // Messages with "tmi.twitch.tv" as a prefix..
            ...
            switch(message.command) {
                ...
                // Handle subscription / resub / subgift / submysterygift / giftpaidupgrade / raid...
                case "USERNOTICE":
                    ...
                    var prime = plan ? plan.includes("Prime") : null;
                    var userstate = message.tags;
                    userstate['message-type'] = message.tags["msg-id"];
                    ...
                    // Handle mass sub gifting
                    else if (msgid === 'submysterygift') {
                        // 'giftCount' is the number of subs gifted for this event
                        var giftCount = _.get(~~message.tags["msg-param-mass-gift-count"], null);
                        // 'senderCount' is the total number of gifts that user has given in this channel
                        // 'senderCount' will be false if the user does not share their total
                        var senderCount = _.get(~~message.tags["msg-param-sender-count"], null);
                        this.emits(["submysterygift", "massgift"], [
                            [channel, username, giftCount, plan, senderCount, userstate],
                            [channel, username, giftCount, plan, senderCount, userstate]
                        ]);
                    }

                    // Handle gift promo notifications such as 'Subtember'
                    else if (msgid === 'giftpaidupgrade') {
                        var promo = message.tags["msg-param-promo-name"];
                        var total = _.get(~~message.tags["msg-param-promo-gift-total"], null);
                        var sender = message.tags["msg-param-sender-name"] || message.tags["msg-param-sender-login"];
                        this.emits(["giftpaidupgrade", "giftupgrade"], [
                            [channel, username, plan, promo, total, sender, userstate],
                            [channel, username, plan, promo, total, sender, userstate]
                        ]);
                    }

                    // Handle a channel raid
                    else if (msgid === 'raid') {
                        // 'raider' is the source channel of the raid
                        var raider = message.tags["msg-param-displayName"] || message.tags["msg-param-login"];
                        var viewerCount = _.get(~~message.tags["msg-param-viewerCount"], null);
                        this.emit("raid", channel, username, raider, viewerCount, userstate);
                    }
                    break;
                ...
                case "PRIVMSG":
                    ...
                    if (message.tags.username === "jtv") {
                        ...
                        if (msg.includes("hosting you for")) {
                            ...
                            this.emit("hosted", channel, _.username(msg.split(" ")[0]), count, msg.includes("auto"), message.tags);
                        }

                        // Some is hosting the channel, but no viewer(s) count provided in the message..
                        else if (msg.includes("hosting you")) {
                            this.emit("hosted", channel, _.username(msg.split(" ")[0]), 0, msg.includes("auto"), message.tags);
                        }
                    }
...
client.prototype._sendMessage = function _sendMessage(delay, channel, message, fn) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (!_.isNull(this.ws) && this.ws.readyState !== 2 && this.ws.readyState !== 3 && !_.isJustinfan(this.getUsername())) {
            ...
            //console.log(message.substring(500))
