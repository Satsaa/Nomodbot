...
module.exports.run = (channel, userstate, params) => {
  return new Promise((resolve, reject) => {
      ...
      if (err) { // create channel response base
        fs.writeFile('./data/' + channel + '/responses.json', '{}', (err) => {
          if (!err) {
            ...
            console.error(`* [${channel}] FAILED TO CREATE RESPONSE FILE: ${err}`)
          }
        })
    ...
    function main (channel, userstate, params) {
      let local = nmb.bot[channel].responses // reference for neat code
      if (params[2] && params[2].charAt(0) !== '?' && !nmb.bot.config.masters.includes(userstate['username'])) return 'Insufficient permissions to edit responses without \'?\' prefix!'
      ...
      if (typeof params[1] !== 'undefined') {
        if (params[1].toLowerCase() === 'add') { // add a response
          ...
          if (!nmb.bot.config.masters.includes(userstate['username'])) return 'Insufficient permissions to delete responses'
          ...
          if (!nmb.bot.config.masters.includes(userstate['username'])) return 'Insufficient permissions to edit responses'
          if (!params[2]) return 'You must specify a command name! (param 2)'
          if (!params[3]) return 'You must specify a response! (param 3+)'
