...
export default class ParamValidator {
  ...
  public async validate(channelId: number, pluginId: string, group: string | undefined, words: string[]): Promise<{pass: false, message: string} | {pass: true, replace: string[]}> {
      ...
      if (check.pass) {
        ...
        if (Object.keys(users).length) {
          ...
          for (const user in users) {
            if (res[user]) replace[users[user]] = `${res[user]}` // Change login to id
          ...
          for (const index of duplicateUserIndexes) {
            ...
            if (res[user]) replace[index] = `${res[user]}`
  ...
  public check(cmdParams: CommandParameter[], words: string[], channelId?: number): { pass: boolean, fields: boolean[] } {
      ...
      if (typeof field.pure === 'object') { // Tuple
        ...
        if (word) {
          if (field.var) { // Variable
            if (!types(field, word)) return false
          } else if (field.pure !== (field.case ? word : word.toLowerCase())) { return false }
        }
      } else if (!word) {
        return false
      } else if (field.var) { // Variable
    ...
    function types(field: CommandParameter, word: string): boolean {
      ...
      function switcheroo(regex: RegExp | undefined, pure: string, word: string) {
        if (regex) return Boolean(word.match(regex))
        switch (pure) {
          case 'NUMBER':
          case 'NUMBERS':
