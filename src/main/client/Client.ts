...
interface Events {
  ...
  chat: (channelId: number, userId: number, userstate: PRIVMSG['tags'], message: string, me: boolean, self: boolean, irc: PRIVMSG | null) => void
  ...
  // websocket
  wsopen: (ws: WebSocket | undefined) => void
  wsmessage: (ws: WebSocket | undefined) => void
  wserror: (ws: WebSocket | undefined) => void
  wsclose: (ws: WebSocket | undefined) => void
}
...
export default class TwitchClient {
  ...
  constructor(options: TwitchClientOptions) {
    ...
    try {
      ...
      if (err.code === 'ENOENT')
        fs.writeFileSync(`${this.opts.dataRoot}/global/${this.opts.dataFile}`, '{}')
      else
        throw err
    }
    ...
    this.rateLimiter.times.forEach((v, i) => {
      if (!this.clientData.global.msgTimes[i])
        this.clientData.global.msgTimes.push([])
    })
    this.whisperRateLimiter.times.forEach((v, i) => {
      if (!this.clientData.global.whisperTimes[i])
        this.clientData.global.whisperTimes.push([])
    })
    ...
    try {
      ...
      if (err.code === 'ENOENT')
        fs.writeFileSync('./misc/seenMessageTypes.json', '{}')
      else
        throw err
    }
  ...
  public get joinedChannels(): number[] {
    return Object.keys(this.channels).map(v => parseInt(v))
  }
  ...
  public chat(channelId: number, msg: string, options: {command?: boolean} = {}) {
    return new Promise((resolve) => {
      ...
      if (!login || !this.clientData.channels[channelId])
        return resolve(false)
      if (msg.length > this.opts.maxMsgLength)
        msg = msg.slice(0, this.opts.maxMsgLength)
      // Remove chatterino shit
      if (msg.endsWith(this.opts.dupeAffix))
      ...
      this.rateLimiter.queue(async() => {
        ...
        if (this.clientData.channels[channelId].phase && isCommand)
          msg += this.opts.dupeAffix
        ...
        const res = await eventTimeout(this, isCommand ? 'notice' : 'userstate', {
          ...
          matchArgs: isCommand
            ? undefined
            : [channelId, { 'display-name': this.globaluserstate['display-name'] }],
        })
        if (res.timeout) {
          ...
          if (!botId) {
            this.failHandle(undefined, 'BOT ID')
            return
    ...
    return new Promise(async(resolve) => {
      ...
      if (login) {
        this.whisperRateLimiter.queue(() => {
          this.send(`PRIVMSG #${this.opts.username} :/w ${userId} ${msg.replace(/\n/g, '')}`)
          resolve(true)
        })
      } else {
        resolve(false)
  ...
  private onOpen(event: { target: WebSocket }) {
    ...
    // Before login so globaluserstate is received
  ...
  private onMessage(event: { data: WebSocket.Data, target: WebSocket, type: string }) {
    this.emit('wsmessage', this.ws)
  ...
  private doc(msg: IrcMessage) {
    if (msg.cmd !== null) {
      ...
      if (msg.cmd === 'NOTICE' && typeof msg.tags['msg-id'] === 'string') {
        ...
        if (!this.messageTypes.userNoticeIds)
          this.messageTypes.userNoticeIds = {}
        if (this.messageTypes.userNoticeIds[tag3])
          this.messageTypes.userNoticeIds[tag3].__count__++
        else
          this.messageTypes.userNoticeIds[tag3] = { __count__: 1 }
        ...
        const tag3 = msg.tags['msg-id'].toString()
        // !!! Inspect reward gifts
        if (tag3 === 'rewardgift') {
          console.error('rewardgift!')
          console.log(msg)
        }
        if (!this.messageTypes.noticeIds)
          this.messageTypes.noticeIds = {}
        if (this.messageTypes.noticeIds[tag3])
          this.messageTypes.noticeIds[tag3].__count__++
        else
          this.messageTypes.noticeIds[tag3] = { __count__: 1 }
      ...
      // commands
      if (!this.messageTypes.commands)
        this.messageTypes.commands = {}
      if (this.messageTypes.commands[msg.cmd])
        this.messageTypes.commands[msg.cmd].__count__++
      else
        this.messageTypes.commands[msg.cmd] = { __count__: 1 }
      ...
      if (!commandObj._paramlengths_)
        commandObj._paramlengths_ = {}
      if (commandObj._paramlengths_[msg.params.length])
        commandObj._paramlengths_[msg.params.length]++
      else
        commandObj._paramlengths_[msg.params.length] = 1

      if (!commandObj._prefix_)
        commandObj._prefix_ = {}
      if (msg.prefix)
        commandObj._prefix_.prefix++
      else
        commandObj._prefix_.prefix = 1
      if (msg.user)
        commandObj._prefix_.user++
      else
        commandObj._prefix_.user = 1
      if (msg.nick)
        commandObj._prefix_.nick++
      else
        commandObj._prefix_.nick = 1
    }
  }

  private onError(event: { error: any, message: string, target: WebSocket, type: string }) {
    console.error(event.error)
    this.emit('wserror', this.ws)
    this.ws = undefined
    this.reconnect()
  }

  private onClose(event: { code: number, reason: string, target: WebSocket, wasClean: boolean }) {
    console.log(`Connection closed: ${event.code}, ${event.reason}`)
    this.emit('wsclose', this.ws)
    this.ws = undefined
    this.ids = {}
    this.channels = {}
    this.reconnect()
  ...
  private onExit() {
    if (!this.clientData)
      return console.warn('[Client] clientData not saved due to it being undefined!')
    try {
    ...
    function replacer(k: string, v: any) {
      return ['userstate'].indexOf(k) === -1 ? v : undefined
    }
  }
  ...
  private async reconnect(interval: number = this.opts.reconnectInterval): Promise<void> {
    if (interval < 1000)
      throw new Error(`Too small interval: ${interval}`)
    if (this.opts.maxReconnects < this.reconnects++)
      throw new Error('Too many unsuccessful reconnects')
  ...
  private async mod(channelId: number, user: string, mod: boolean) {
    if (!this.channelCache.mods[channelId])
      this.channelCache.mods[channelId] = {}
    if (mod) {
      if (this.channelCache.mods[channelId][user])
        return
      else
        this.channelCache.mods[channelId][user] = true
  ...
  private async userJoin(channelId: number, user: string) {
    if (!this.channelCache.users[channelId])
      this.channelCache.users[channelId] = {}
    if (this.channelCache.users[channelId][user])
  ...
  private async handleMessage(msg: IrcMessage) {
    ...
    switch (msg.cmd) {
      ...
      case '353': // :nomodbot.<prefix> 353 <you> = #<channel> :<user1> <user2> ... <userN>
        ...
        for (const user of msg.params[3].split(' '))
          this.userJoin(channelId, user)
        break
      case 'CAP':
        break
      ...
      case 'CLEARCHAT':
        ...
        if (!userId)
          return this.failHandle(msg, msg.cmd)
        if (typeof msg.tags['ban-duration'] === 'number')
          this.emit('timeout', channelId, userId, msg.tags['ban-duration'])
        else
      ...
      case 'ROOMSTATE': // <tags> :tmi.twitch.tv ROOMSTATE #<channel>
        ...
        if (msg.tags['emote-only'] === 1)
          this.ircLog(`${channel} is in emote only mode`)
        if (msg.tags['followers-only'] !== -1)
          this.ircLog(`${channel} is in follower only mode (${msg.tags['followers-only']})`)
        if (msg.tags['subs-only'] === 1)
          this.ircLog(`${channel} is in subscriber only mode`)
        if (msg.tags.slow === 1)
          this.ircLog(`${channel} is in slow mode`)
      ...
      case 'USERSTATE': // <tags> <prefix> USERSTATE #<channel>
        ...
        if ((msg.tags || {}).badges)
          this.mod(channelId, this.opts.username, Boolean(msg.tags.badges!.moderator))
      ...
      case 'HOSTTARGET':
        ...
        if (msg.params[1]) {
          ...
          if (!targetId)
            return this.failHandle(msg, msg.cmd)
          this.emit('hosttarget', channelId, targetId, msg.params[2] === undefined ? null : ~~msg.params[2])
        } else {
          this.emit('hosttarget', channelId, null, msg.params[2] === undefined ? null : ~~msg.params[2])
        }
      ...
      case 'USERNOTICE': // <tags> <prefix> USERNOTICE #<channel> :<message>
        ...
        switch (msg.tags['msg-id']) {
          case 'resub':
          case 'sub': {
            userId = Number(msg.tags['user-id']!)
            if (!userId)
              return this.failHandle(msg, 'Can\'t get userId of sub notice')
          ...
          case 'raid': {
            ...
            if (!id)
              return this.failHandle(msg, msg.tags['msg-id'])
          ...
          case 'ritual':
            ...
            if (!id)
              return this.failHandle(msg, msg.tags['msg-id'])
            this.emit('ritual', channelId, id, String(msg.tags['msg-param-ritual-name']), msg.params[1])
            break
      ...
      case 'NOTICE': // <tags> <prefix> NOTICE #<channel> :<message>
        ...
        if (!channelId) {
          ...
          if (last === 'Login authentication failed' || last === 'Login unsuccessful' || last === 'Improperly formatted auth') {
            console.error(last)
            process.exit(1)
          }
          return this.failHandle(msg, msg.cmd)
        ...
        if (msg.tags['msg-id'] === 'msg_ratelimit')
        ...
        switch (msg.tags['msg-id']) {
          case 'msg_ratelimit':
            ...
            break
          case 'msg_timedout': {
            const duration = typeof msg.params[1] === 'string'
              ? ~~msg.params[1].match(/([0-9]*)[a-zA-Z .]*$/)![1]
              : 0
